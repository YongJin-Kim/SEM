...
{
    public partial class ImageTrackBar : Control
    {

        private Rectangle leftRect;
        private Rectangle rightRect;
        private Rectangle panelRect;
        private Rectangle trackBarRect;
        private Rectangle reBruRect;
        private Rectangle TextRect;

        //private Rectangle vaRect;
        ...
        //private Image _ValueIcon = null;
        //public Image ValueIcon
        //{
        //    get { return _ValueIcon; }
        //    set
        //    {
        //        _ValueIcon = value;
        //        this.Invalidate(vaRect);
        //    }
        //}
        ...
        private Image _TrackBarIcon = null;
        public Image TrackBarIcon
        {
            get { return _TrackBarIcon; }
            set
            {
                _TrackBarIcon = value;
                this.Invalidate(trackBarRect);
            }
        }

        private Image _reBruIcon = null;
        public Image reBruIcon
        {
            get { return _reBruIcon; }
            set
            {
                _reBruIcon = value;
                this.Invalidate(reBruRect);
            }
        }
        ...
        private Color revBru = Color.Transparent;
		...
		/// 표시할 문자열을 사용자가 지정 한다.
		/// </summary>
		public Func<ImageTrackBar,string> UserValueString;
		...
		protected override void OnPaint(PaintEventArgs e)
		{
            ...
            //Helper.GuiPainter.DrawButtonEllipse(g, vaRect, _ButtonRightStyle.BackColor, _ButtonRightStyle.ColorStart, _ButtonRightStyle.ColorCenter, rightBswm);
            ...
            //g.DrawImage(_TrackBarIcon, this.ClientRectangle);
            g.DrawImage(_TrackBarIcon, panelRect);
            ...
            //if (_ValueIcon != null)
            //{
            //    Rectangle vRect = vaRect;
            //    vRect.Inflate((int)(rightRect.Width * (_ButtonRightStyle.PaintRatio / 100d - 1d)), (int)(rightRect.Height * (_ButtonRightStyle.PaintRatio / 100d - 1d)));
            //    vRect.Offset(_ButtonRightStyle.ImageOffset);
            //    g.DrawImage(_RightIcon, vRect);
            //}
			...
			if (_IndicatorVisiable)
			{
				...
				try
				{
                    ...
                    textRect = new Rectangle(4 + (_Value - _Minimum) * (panelRect.Width - 8) / gapMaxMain + panelRect.Left - 2, -8, 45, 25);
                    
                    if (!this.Orientation)
                    {
                        valueRect = new Rectangle(4 + (_Value - _Minimum) * (panelRect.Width - 8) / gapMaxMain + panelRect.Left - 8, 2, 14, 14);
                        textRect = new Rectangle(4 + (_Value - _Minimum) * (panelRect.Width - 8) / gapMaxMain + panelRect.Left - 8, -8, 45, 25);
                        
                    }
                    else if (this.Orientation)
                    {
                        valueRect = new Rectangle(0 ,4 + (_Minimum - _Value) * (panelRect.Height - 27) / gapMaxMain + (panelRect.Bottom - 32) - 2, 27, 6);
                        //textRect = new Rectangle(0, 4 + (_Minimum - _Value) * (panelRect.Height - 27) / gapMaxMain + (panelRect.Bottom - 32) - 2-25, 45, 25);
                        //valueRect = new Rectangle(4 + (_Value - _Minimum) * (panelRect.Width - 8) / gapMaxMain + panelRect.Left - 2, -2, 14, 27);
				...
				catch (DivideByZeroException)
				{
                    ...
                    textRect = new Rectangle(panelRect.Left + panelWidthHalf - 2, -8, 45, 25);
                    
				}

                //if (!this.Orientation)
               // {
                    reRect = new Rectangle(leftRect.Right, panelRect.Top + 10, valueRect.Left - (valueRect.Width), panelRect.Bottom / 8);
                   
                //}
                //else
                //{
                //    //reRect = new Rectangle(leftRect.Left, leftRect.Top, leftRect.Width, leftRect.Top - valueRect.Height);
                ...
                position = valueRect;
                //g.FillRectangle(new SolidBrush(Color.FromArgb(50, 235, 251)), reRect);
                ControlPaint.DrawBorder(g, reRect, ControlPaint.LightLight(revBru), ButtonBorderStyle.None);
                g.DrawImage(_reBruIcon, valueRect);
                //if (!this.Orientation)
                //{
                //    g.DrawImage(_TextIcon, textRect);
                //}
		...
		private void CalControlBounce()
		{
			if (_ButtonSizeAuto)
			{
                if (!_orientation)
                {
                    ...
                    //trackBarRect = new Rectangle(leftRect.Right, this.ClientRectangle.Top, TrackBarIcon.Width - 1, TrackBarIcon.Height - 1);
                    rightRect = new Rectangle(panelRect.Right - leftRect.Width, this.ClientRectangle.Top, leftRect.Width, this.ClientRectangle.Height - 1);
                    panelRect = new Rectangle(leftRect.Right + Padding.Horizontal, this.ClientRectangle.Top, this.ClientRectangle.Width - rightRect.Width, this.ClientRectangle.Height);
                    //panelRect = new Rectangle(leftRect.Right + Padding.Horizontal, this.ClientRectangle.Top, this.ClientRectangle.Width - rightRect.Width, this.ClientRectangle.Height);
                }
                else
                {
                    leftRect = new Rectangle(this.ClientRectangle.Left, this.ClientRectangle.Bottom - this.ButtonSize.Height, rightRect.Width -1, rightRect.Height-1);
                    rightRect = new Rectangle(this.ClientRectangle.Left, this.ClientRectangle.Top, this._ButtonSize.Width - 1, this._ButtonSize.Height - 1);
                    panelRect = new Rectangle(this.ClientRectangle.Left, this.rightRect.Bottom, rightRect.Width, this.ClientRectangle.Height - rightRect.Height);
                }
			...
			else
			{
                if (!_orientation)
                {
                    ...
                    //trackBarRect = new Rectangle(leftRect.Right, this.ClientRectangle.Top, TrackBarIcon.Width - 1, TrackBarIcon.Height - 1);
                    panelRect = new Rectangle(leftRect.Right, this.ClientRectangle.Top, (this.ClientRectangle.Width - rightRect.Width) - leftRect.Width, this.ClientRectangle.Height);
                    rightRect = new Rectangle(panelRect.Right, this.ClientRectangle.Top, leftRect.Width, leftRect.Height - 1);
                }
                else
                {
                    leftRect = new Rectangle(this.ClientRectangle.Left, this.ClientRectangle.Bottom - this.ButtonSize.Height, this.ButtonSize.Width - 1, this.ButtonSize.Height -1);
                    rightRect = new Rectangle(this.ClientRectangle.Left, this.ClientRectangle.Top, this._ButtonSize.Width - 1, this._ButtonSize.Height - 1);
                    panelRect = new Rectangle(this.ClientRectangle.Left, rightRect.Bottom, rightRect.Width, this.ClientRectangle.Height - rightRect.Height);
                }
		...
		protected override void OnEnabledChanged(EventArgs e)
		{
			if(this.Enabled)
			{
                ...
                {
                    //if (this.Orientation) { rightBswm &= ~ButtonStatesWithMouse.Disabled; }
                    //else { leftBswm &= ~ButtonStatesWithMouse.Disabled; }
                ...
                {
                    //if (this.Orientation) { leftBswm &= ~ButtonStatesWithMouse.Disabled; }
                    //else { rightBswm &= ~ButtonStatesWithMouse.Disabled; }
